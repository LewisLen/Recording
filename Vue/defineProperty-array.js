...
function defineReactive(data,key,val = data[key]){
  ...
  // 被defineReactive化的属性都通过闭包引用属于自己的 Dep 实例
  observe(val);
...
class Watcher{
  ...
  constructor(data,objPath,callback){
    this._data = data; // 每个 watcher 实例都有_data属性，保存当前观察的数据对象
